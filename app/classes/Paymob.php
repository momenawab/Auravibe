...
class Paymob {
    ...
    public function authenticate() {
        ...
        if ($response && isset($response['token'])) {
            ...
            error_log("Paymob Authentication: SUCCESS");
            return $this->authToken;
        }

        error_log("Paymob Authentication FAILED: " . json_encode($response));
        return false;
    }
    ...
    public function registerOrder($orderData) {
        ...
        $data = [
            ...
            'amount_cents' => (int)$orderData['amount_cents'], // Amount in cents
            'currency' => PAYMOB_CURRENCY,
            'merchant_order_id' => $orderData['merchant_order_id'] ?? time(),
            'items' => $orderData['items'] ?? []
        ];

        error_log("Paymob Register Order Request: " . json_encode($data));
        $response = $this->makeRequest(PAYMOB_ORDER_URL, $data);

        if ($response && isset($response['id'])) {
            error_log("Paymob Order Registration: SUCCESS - Order ID: " . $response['id']);
        } else {
            error_log("Paymob Order Registration FAILED: " . json_encode($response));
        }
    ...
    public function getPaymentKey($paymentData) {
        ...
        $data = [
            'auth_token' => $this->authToken,
            'amount_cents' => (int)$paymentData['amount_cents'],
            'expiration' => 3600, // Token expires in 1 hour
            'order_id' => $paymentData['order_id'],
            'billing_data' => $paymentData['billing_data'],
            'currency' => PAYMOB_CURRENCY,
            'integration_id' => (int)$paymentData['integration_id']
        ];

        // Log request for debugging
        error_log("Paymob Payment Key Request: " . json_encode($data));
        ...
        if (isset($response['detail'])) {
            error_log("Paymob Payment Key Error: " . json_encode($response['detail']));
            
            // Check for common errors
            if (is_array($response['detail']) && in_array('Invalid Payment method integration', $response['detail'])) {
                error_log("PAYMOB CONFIG ERROR: Integration ID {$data['integration_id']} is invalid. Check your Paymob dashboard for correct Integration IDs.");
                error_log("SOLUTION: Log in to https://accept.paymob.com → Developers → Integrations, and update PAYMOB_INTEGRATION_* values in .env file");
            }
        } else {
            error_log("Paymob Payment Key Error: " . json_encode($response));
        }
